var init = () => {
    ...
    // WebGL Hyperspace warp — Star Wars Lightspeed Jump
    {
        ...
        var warpVS = [
            ...
            '  float depth = 1.0 - z;',
            '',
            '  // Spiral tünel rotasyonu — hyperspace sırasında yıldızlar döner',
            '  float swirlAngle = angle + uSwirl * depth * 1.2;',
            '',
            ...
            '',
            '  // Yıldız çizgileri — hız arttıkça dramatik uzama',
            '  float sf = smoothstep(0.0, 30.0, uSpeed);',
            '  float sLen = min(max(0.001, sf * uSpeed * 0.009 * depth * ps), 4.5);',
            '  float sWid = mix(0.0015, 0.00025, sf) * (0.4 + depth * 0.9);',
            '',
            '  // Yüksek hızda çizgiler daha kalın',
            '  float thicken = smoothstep(80.0, 200.0, uSpeed) * 1.8;',
            '  sWid *= (1.0 + thicken);',
            '',
            ...
            '  // Yüksek hızda daha parlak',
            '  vAlpha *= (1.0 + smoothstep(80.0, 200.0, uSpeed) * 0.8);',
            '  vT = position.x;',
            '  vBright = bright;',
            '  vDepth = depth;',
            '',
        ...
        var warpFS = [
            ...
            '',
            'void main() {',
            '  float fade = 1.0 - vT * 0.5;',
            '  float a = vAlpha * fade;',
            '',
            '  // Star Wars renk paleti: derin mavi → cyan → beyaz çekirdek',
            '  vec3 deepBlue = vec3(0.15, 0.25, 0.85);',
            '  vec3 cyan     = vec3(0.3, 0.75, 1.0);',
            '  vec3 white    = vec3(0.9, 0.95, 1.0);',
            '',
            '  // Derinliğe göre renk geçişi: uzak=mavi, yakın=cyan, parlak=beyaz',
            '  vec3 baseCol = mix(deepBlue, cyan, vDepth);',
            '  baseCol = mix(baseCol, white, vBright * vBright * 0.7);',
            '',
            '  // Enerji çekirdeği — çizginin başlangıcı daha parlak ve beyaz',
            '  float core = (1.0 - vT) * (1.0 - vT);',
            '  baseCol = mix(baseCol, white, core * 0.5);',
            '  a *= (1.0 + core * 0.4);',
            '',
        ...
        // Arka plan: Hyperspace tunnel — Star Wars sinematik
        ...
        var bgFS = [
            ...
            'uniform float uEntryFlash;',
            'varying vec2 vP;',
            'void main() {',
            '  float d = length(vP);',
            '',
            '  // Giriş flash — merkeze yoğun beyaz-mavi patlama',
            '  float entry = uEntryFlash * exp(-d * d * 1.5);',
            '  vec3 entryCol = mix(vec3(0.7, 0.85, 1.0), vec3(1.0), entry * 0.5);',
            '',
            '  // Çıkış flash — ekranı beyaza boyar',
            '  float exitF = uFlash * exp(-d * d * 0.8);',
            '  vec3 exitCol = vec3(0.95, 0.97, 1.0) * exitF;',
            '',
            '  // Hyperspace tunnel vignette — kenarlar karanlık, merkez parlak mavi',
            '  float vign = smoothstep(0.2, 1.4, d);',
            '  float tunnelGlow = (1.0 - vign) * uTunnel;',
            '  vec3 tunnelCol = mix(vec3(0.15, 0.25, 0.6), vec3(0.5, 0.7, 1.0), 1.0 - d) * tunnelGlow;',
            '',
            '  // Blue shift — hyperspace sırasında tüm sahneye mavi renk kayması',
            '  vec3 blueShift = vec3(0.05, 0.12, 0.35) * uBlueShift;',
            '',
            '  // Merkez ışık halkası — tünelin uç noktası',
            '  float ring = exp(-pow(d - 0.05, 2.0) * 80.0) * uTunnel * 0.6;',
            '  vec3 ringCol = vec3(0.8, 0.9, 1.0) * ring;',
            '',
            '  vec3 col = entryCol * entry + exitCol + tunnelCol + blueShift + ringCol;',
            '  float alpha = max(max(uAlpha, entry * 0.7), max(exitF * 0.5, tunnelGlow * 0.8));',
            '  alpha = max(alpha, uBlueShift * 0.15);',
        ...
        var bgMat = new THREE.ShaderMaterial({
            uniforms: {
                uAlpha: { value: 0.0 },
                uFlash: { value: 0.0 },
                uBlueShift: { value: 0.0 },
                uTunnel: { value: 0.0 },
                uEntryFlash: { value: 0.0 }
            },
